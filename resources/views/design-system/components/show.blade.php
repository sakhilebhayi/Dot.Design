<x-app-layout>
<x-slot name="header">{{ $component->name }}</x-slot>
...
<div style="padding:2rem 2.5rem;max-width:1400px;">
    ...
    <div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:1.5rem;">
        <span style="font-size:0.75rem;color:var(--text-dim);text-transform:uppercase;letter-spacing:0.06em;">{{ $component->category }}</span>
    ...
    <div class="dot-card" style="padding:1.5rem;margin-bottom:1.5rem;">
        <p style="font-size:0.85rem;color:var(--text-muted);max-width:70ch;margin:0;line-height:1.6;">
            {{ $component->description ?: 'No description recorded for this component.' }}
        </p>
    </div>
    ...
    @if ($component->props_schema)
        ...
        <pre class="dot-card" style="padding:1rem;color:var(--text-muted);font-size:0.8rem;overflow-x:auto;margin:0;">{{ json_encode($component->props_schema, JSON_PRETTY_PRINT) }}</pre>
    @else
